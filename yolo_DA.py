...
try:
    ...
    print(f"    YOLO load time: {time.time() - t0:.4f}s")

    # Preprocess YOLO
    t1 = time.time()
    yolo_input, orig_img = preprocess_yolo(IMAGE_PATH, YOLO_INPUT_SIZE)
    orig_h, orig_w = orig_img.shape[:2]
    print(f"    YOLO preprocess time: {time.time() - t1:.4f}s")

    # Run YOLO inference
    t2 = time.time()
    yolo_output = yolo_sess.run(None, {yolo_input_name: yolo_input})[0]
    print(f"   YOLO inference time: {time.time() - t2:.4f}s")
    ...
    print(f"   YOLO postprocess time: {time.time() - t3:.4f}s")
    ...
    print(f"   DepthAnything load time: {time.time() - t4:.4f}s")

    t5 = time.time()
    dav2_input = preprocess_dav2(IMAGE_PATH, DAV2_INPUT_SIZE, DAV2_INPUT_SIZE)
    print(f"   DepthAnything preprocess time: {time.time() - t5:.4f}s")

    t6 = time.time()
    dav2_output = dav2_sess.run(None, {dav2_input_name: dav2_input})[0]
    print(f"   DepthAnything inference time: {time.time() - t6:.4f}s")
    ...
    print(f"\nTOTAL pipeline time: {time.time() - total_start:.4f}s")
